...
setMethod(cyto_plot_gate, signature = "rectangleGate", definition = function(x, 
  ...
  if (length(gt@min) == 2) {
    ...
    if (gate_point == TRUE) {
      points(x = c(gt@min[channels[1]], gt@max[channels[1]]), y = c(hln, hln), col = gate_line_col, pch = gate_point_shape, cex = gate_point_size)
    }
...
setMethod(cyto_plot_gate, signature = "list", definition = function(x, 
  ...
  # Plot Gates
  mapply(function(gt, gate_line_col, gate_line_type, gate_line_width, gate_point_shape, gate_point_size) {
