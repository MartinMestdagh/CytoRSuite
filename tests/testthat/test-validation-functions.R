...
test_that("checkFile", {
  expect_false(checkFile("Test.csv"))
})
...
test_that("checkOverlay flowSet method returns list of flowFrame lists", {
  ...
  expect_error(checkOverlay(fs, overlay = list(list(fs[[1]]), list(fs[[2]]))), "Overlay should be a list of flowFrames lists to overlay on each flowFrame in the flowSet.")
})

test_that("checkOverlay GatingHierarchy method returns list of flowFrames", {
  expect_error(checkOverlay(gs[[1]], overlay = "TEST"), "Supplied population(s) for overlay does not exist in the GatingHierarchy.", fixed = TRUE)

  ov <- list(getData(gs, "T Cells")[[1]])
  names(ov) <- "T Cells"

  expect_equal(checkOverlay(gs[[1]], overlay = "T Cells"), ov)
  expect_equal(checkOverlay(gs[[1]], overlay = getData(gs, "T Cells")[[4]]), list(getData(gs, "T Cells")[[4]]))
})

test_that("checkOverlay GatingSet method returns list of flowFrame lists", {
  expect_error(checkOverlay(gs, overlay = "TEST"), "Supplied population(s) for overlay does not exist in the GatingHierarchy.", fixed = TRUE)

  TC <- getData(gs, "T Cells")
  ov <- list(list(TC[[1]]), list(TC[[2]]), list(TC[[3]]), list(TC[[4]]))

  expect_equivalent(checkOverlay(gs, overlay = "T Cells"), ov)
})
...
test_that("checkStat stops computeStats if supplied stat is not supported", {
  ...
  expect_equal(checkStat("Mean"), "mean")
  expect_equal(checkStat("Median"), "median")
  expect_equal(checkStat("Mode"), "mode")
  expect_equal(checkStat("Count"), "count")
  expect_equal(checkStat("Freq"), "freq")
  expect_equal(checkStat("Geo mean"), "geo mean")
  expect_equal(checkStat("cv"), "CV")
  expect_equal(checkStat("cvi"), "CVI")
})
