...
test_that("computeStats flowFrame method returns the correct statistics",{
  ...
  expect_equal(computeStats(Va2[[1]], stat = "median"), sts, tolerance = 0.01)
  ...
  expect_equal(computeStats(Va2[[1]], stat = "median", channels = c("CD4","CD8")), sts, tolerance = 0.01)
  
  sts <- matrix(colMedians(exprs(V[[1]])), nrow = 1, ncol = length(chans))
  rownames(sts) <- nms[[1]]
  colnames(sts) <- chans
  
  expect_equal(computeStats(Va2[[1]], stat = "median", transList = trans), sts, tolerance = 0.01)
  ...
  expect_equal(computeStats(Va2[[1]], stat = "mean"), sts, tolerance = 0.01)
  ...
  expect_equal(computeStats(Va2[[1]], stat = "mean", channels = c("CD4","CD8")), sts, tolerance = 0.01)
  
  sts <- matrix(colMeans(exprs(V[[1]])), nrow = 1, ncol = length(chans))
  rownames(sts) <- nms[[1]]
  colnames(sts) <- chans
  
  expect_equal(computeStats(Va2[[1]], stat = "mean", transList = trans), sts, tolerance = 0.01)
  ...
  expect_equal(computeStats(Va2[[1]], stat = "geo mean", transList = trans), sts, tolerance = 0.01)
  ...
  expect_error(computeStats(Va2[[1]], stat = "geo mean"),"Some channels have been transformed, supply the transformList to calculate the geometric mean.")
  ...
  expect_equal(computeStats(Va2[[1]], stat = "mode"), sts, tolerance = 0.01)
  
  sts <- matrix(sts[, c("Alexa Fluor 700-A", "Alexa Fluor 488-A")], nrow = 1)
  rownames(sts) <- nms[[1]]
  colnames(sts) <- c("Alexa Fluor 700-A", "Alexa Fluor 488-A")
  
  expect_equal(computeStats(Va2[[1]], stat = "mode", channels = c("CD4","CD8")), sts, tolerance = 0.01)
  ...
  expect_equal(computeStats(Va2[[1]], stat = "mode", transList = trans), sts, tolerance = 0.01)
  ...
  expect_equal(computeStats(Va2[[1]], stat = "CV"), sts, tolerance = 0.01)
  
  sts <- matrix(sts[, c("Alexa Fluor 700-A", "Alexa Fluor 488-A")], nrow = 1)
  rownames(sts) <- nms[[1]]
  colnames(sts) <- c("Alexa Fluor 700-A", "Alexa Fluor 488-A")
  
  expect_equal(computeStats(Va2[[1]], stat = "CV", channels = c("CD4","CD8")), sts, tolerance = 0.01)
  ...
  expect_equal(computeStats(Va2[[1]], stat = "CV", transList = trans), sts, tolerance = 0.01)
  ...
  expect_equal(computeStats(Va2[[1]], stat = "CVI"), sts, tolerance = 0.01)
  
  sts <- matrix(sts[, c("Alexa Fluor 700-A", "Alexa Fluor 488-A")], nrow = 1)
  rownames(sts) <- nms[[1]]
  colnames(sts) <- c("Alexa Fluor 700-A", "Alexa Fluor 488-A")
  
  expect_equal(computeStats(Va2[[1]], stat = "CVI", channels = c("CD4","CD8")), sts, tolerance = 0.01)
  ...
  expect_equal(computeStats(Va2[[1]], stat = "CVI", transList = trans), sts, tolerance = 0.01)
  ...
  expect_equal(computeStats(Va2[[1]], stat = "count"), sts, tolerance = 0.01)
...
test_that("computeStats flowSet method returns the correct statistics",{
  ...
  expect_equal(computeStats(Va2, stat = "count", transList = trans)[[1]], sts, tolerance = 0.01)
  
  sts <- fsApply(V, colMedians, use.exprs = TRUE)
  sts <- cbind(pData(Va2), sts)
  sts <- sts[,-1]
  
  expect_equal(computeStats(Va2, stat = "median", transList = trans)[[1]], sts, tolerance = 0.01)
...
test_that("computeStats GatingSet method returns the correct statistics",{
  
  sts <- fsApply(V, colMedians, use.exprs = TRUE)
  sts <- cbind(pData(Va2), sts)
  sts <- sts[,-1]
  
  expect_equal(computeStats(gs, alias = "T Cells", stat = "median", transList = trans, save = FALSE)[[1]], sts)
  
  expect_equal(computeStats(gs, alias = "T Cells", stat = "median", save = FALSE)[[1]], sts)
